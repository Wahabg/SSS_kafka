...
# Apply analytical and ranking functions with rollup and cube
result_df = df \
  .withWatermark("last_updated", "1 minute") \
  .rollup("symbol", "minute") \
  .agg(avg("price_usd").alias("avg_price")) \
  .orderBy("symbol", "minute") \
  .selectExpr("COALESCE(symbol, 'Total') as symbol", "minute", "avg_price")
output_query = result_df \
        .writeStream \
        .format("console")\
        .outputMode("complete")\
        .start()
